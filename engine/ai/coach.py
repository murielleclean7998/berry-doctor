...
class StrawberryCoach:
    ...
    def build_today_tasks(self) -> str:
        stage = self._current_stage()
        tasks = list(self.knowledge_graph.tasks_for_today(variety=self.config.variety))
        weather = self.weather_service.latest()
        ...
        if sensor and sensor.get("humidity") and float(sensor["humidity"]) >= humidity_warning:
            tasks = tasks[:2] + ["습도가 높으니 꽃잎과 곰팡이 흔적을 먼저 확인해 주세요."]
        if sensor and sensor.get("solution_ec") is not None:
            ...
            tasks = tasks[:2] + [f"양액 EC {sensor['solution_ec']} / pH {ph_label} 상태를 보고 조정 여부를 확인해 주세요."]
        while len(tasks) < 3:
            tasks.append("하우스를 한 번 둘러보며 상태를 확인해 주세요.")
        return (
            f"오늘 할 일\n"
            ...
            f"2. {tasks[1]}\n"
            f"3. {tasks[2]}\n"
            f"이유: {self.knowledge_graph.why_for_today(variety=self.config.variety)}"
        )
    ...
    def build_daily_report(self, now: datetime | None = None) -> str:
        ...
        return (
            ...
            f"- 내일: {weather.get('tomorrow_summary')}\n"
    ...
    def build_monthly_report(self, now: datetime | None = None) -> str:
        ...
        return (
            f"{month_key} 월간 리포트\n"
            f"- 누적 수확: {yield_summary['monthly_total_kg']}kg\n"
            f"- 예상 월 수확: {yield_summary['projected_month_kg']}kg\n"
            f"- 예상 시즌 매출: {yield_summary['projected_revenue']:.0f}원\n"
    ...
    def build_year_compare_message(self, house_id: int = 1) -> str:
        ...
        return (
            ...
            f"- 작년 같은 때와 비교: {summary['year_compare']:+.2f}\n"
            f"- 참고: 위성은 바깥에서 본 정보예요."
        )
    ...
    def build_security_history_message(self, days: int = 7) -> str:
        ...
        for event in events[:5]:
            lines.append(
                f"- {event.get('timestamp')} / {event.get('house_id')}동 / 사진 {len(event.get('photo_paths_list', []))}장"
            )
        return "\n".join(lines)
